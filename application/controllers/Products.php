...
class Products extends CI_Controller
{
    ...
    public function product_category()
    {
        ...
        if ($page == 0) {
            ...
            $sh_text = "Showing $start_point to ".count($data['results']).' of '.$this->Products_model->record_category_count().' entries';
        } else {
            $start_sh = $page + 1;
    ...
    public function list_products()
    {
        ...
        if ($page == 0) {
            ...
            $sh_text = "Showing $start_point to ".count($data['results']).' of '.$this->Products_model->record_product_count().' entries';
        } else {
            $start_sh = $page + 1;
            $end_sh = $page + count($data['results']);
        ...
        $data['lang_profit_margin'] = $this->lang->line('profit_margin');
    ...
    public function exportProductsReport()
    {
        ...
        $name = $this->input->get('name');
        $category = $this->input->get('category');
        // $categoryName =   $this->db->query("SELECT * FROM category WHERE id = '$category'")->row()->name;
        ...
        $lang_products_list = $this->lang->line('products_list');
        $lang_date = $this->lang->line('date');

        $lang_product_name = $this->lang->line('product_name');
        $lang_product_code = $this->lang->line('product_code');
        $lang_code = $this->lang->line('code');
        $lang_category = $this->lang->line('category');
        $lang_cost = $this->lang->line('cost');
        $lang_price = $this->lang->line('price');
        $lang_profit_margin = $this->lang->line('profit_margin');
        $lang_status = $this->lang->line('status');
        $lang_filters = $this->lang->line('filters');

        $spreadsheet = new Spreadsheet();
        $spreadsheet->getActiveSheet();
        $sheet = $spreadsheet->getActiveSheet();
        $sheet->mergeCells('A1:G1');
        $sheet->setCellValue('A1', "$lang_products_list");
        ...
        $sheet->getStyle('H1')->applyFromArray($top_header_style);

        $sheet->setCellValue('A2', "$lang_product_code");
        $sheet->setCellValue('B2', "$lang_product_code");
        $sheet->setCellValue('C2', "$lang_product_name");
        $sheet->setCellValue('D2', "$lang_category");
        $sheet->setCellValue('E2', "$lang_cost ($site_currency)");
        $sheet->setCellValue('F2', "$lang_price ($site_currency)");
        $sheet->setCellValue('G2', "$lang_profit_margin ($site_currency)");
        $sheet->setCellValue('H2', "$lang_status ($site_currency)");
        ...
        $sort = '';
        if (!empty($code)) 
            $sort .= " AND code LIKE '$code%' ";
        if (!empty($name))
            $sort .= " AND name LIKE '%$name%' ";
        if (!empty($category)) {
            if ($category == '-') {
                $sort .= ' AND category > 0 ';
            } else {
                $sort .= " AND category = '$category' ";
            }
        }
        ...
        if ($prodRows > 0) {
            ...
            for ($p = 0; $p < count($prodData); ++$p) {
                ...
                $sheet->setCellValue("A$jj", "$id");
                $sheet->setCellValue("B$jj", "$code");
                $sheet->setCellValue("C$jj", "$name");
                $sheet->setCellValue("D$jj", "$category_name");
                
                $sheet->setCellValue("E$jj", "$cost");
                $sheet->setCellValue("F$jj", "$price");
                $sheet->setCellValue("G$jj", "$profit_margin");
                $sheet->setCellValue("H$jj", "$status");
                ...
                $sheet->getStyle("H$jj")->applyFromArray($account_value_style_header);
        ...
        // $sheet->setCellValue("A$jj", "$lang_filters:");
        ...
        $filename = 'export-products-report';
        ob_end_clean();
