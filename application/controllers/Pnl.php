...
class Pnl extends CI_Controller
{
    ...
    public function exportpnlReport()
    {
        ...
        $sheet->setCellValue('A1', "$lang_pnl_report");
        ...
        $sheet->setCellValue('E2', "$lang_grand_total ($site_currency)");
        $sheet->setCellValue('F2', "$lang_cost ($site_currency)");
        $sheet->setCellValue('G2', "$lang_tax ($site_currency)");
        $sheet->setCellValue('H2', "$lang_profit_amount ($site_currency)");

        apply_excel_headers_style($sheet, 7);
        ...
        if ($orderRows > 0) {
            ...
            for ($od = 0; $od < count($orderData); ++$od) {
                ...
                $sheet->setCellValue("F$jj", "$each_sales_cost");
                $sheet->setCellValue("G$jj", "$tax");
                $sheet->setCellValue("H$jj", "$each_profit");

                apply_excel_content_row_style($sheet, $jj, 7);
        ...
        $sheet->setCellValue("E$jj", "$all_grand_amt");
        $sheet->setCellValue("F$jj", "$all_cost_amt");
        $sheet->setCellValue("G$jj", "$all_tax_amt");
        $sheet->setCellValue("H$jj", "$all_profit_amt");

        apply_excel_headers_style($sheet, 7, $jj);
