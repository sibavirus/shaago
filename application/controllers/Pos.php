...
class Pos extends CI_Controller
{
    ...
    public function insertSale()
    {
		...
		<?php
            ...
            $order_pay_amt 				= 0;
            if($vt_status == "1") {
	            $order_pay_amt 			= $grandTotal;
            } else {
	            $order_pay_amt 			= $paid_amt;
            }
            
            $order_pay_number 			= "";
            if( ($paid_by == "3") || ($paid_by == "4")) {		// VISA, MASTER;
	            $order_pay_number 		= $addi_card_numb;
            } else if ($paid_by == "5") {						// Cheque;
	            $order_pay_number 		= $cheque;
            } else if ($paid_by == "7") {						// Gift Card Number;
	            $order_pay_number 		= $card_numb;
            } else {
	            $order_pay_number 		= "";
            }
...
<div id="wrapper">
    ...
    <table class="totals" cellspacing="0" border="0" style="margin-bottom:5px; border-top: 1px solid #000;" width="100%">
        ...
        $mesg .= '
			...
			<tr>    
				...
				<td colspan="2" style="padding-top:5px; text-align:right; font-weight:bold;">'.number_format($total_paid_amt, 2).'</td>
    		</tr>';
		...
		for($op = 0; $op < count($ordPayData); $op++) {
			...
			$ordPay_date 	= date("$site_dateformat", strtotime($ordPayData[$op]->created_datetime));
			...
			<tr>
				<td colspan="3" style="text-align:left; border-top: 1px solid #000; padding-top: 5px; padding-bottom: 5px;">
					<b>'.$this->lang->line("paid_by").' :</b> '.$ordPay_name.' ['.$ordPay_date.']';
					
						if(strlen($display_card_numb) > 0) {
							$mesg	.= "<br />[$display_card_numb]";
						}
						
			$mesg 			.= '	
				</td>
				<td style="text-align:right; border-top: 1px solid #000; padding-top: 5px; padding-bottom: 5px;">'.number_format($ordPay_amt, 2).'</td>
			</tr>
			';
    ...
    <center>
    <div style="border-top:1px solid #000; padding-top:0px;">'.$receipt_footer.'</div>
    </center>
</div>
    ...
    public function loadTodaySales()
    {
        ...
        for($d = 0; $d < count($orderPayData); $d++) {
	        ...
	        if($orderRows == 1) {
		        ...
		        if($orderPay_meth_key == "1") {		// Cash;
			        $total_cash			+= $orderPay_amount;
		        }
		        if($orderPay_meth_key == "2") {		// NETT;
			        $total_nett			+= $orderPay_amount;
		        }
		        if($orderPay_meth_key == "3") {		// VISA;
			        $total_visa 		+= $orderPay_amount;
		        }
		        if($orderPay_meth_key == "4") {		// Master;
			        $total_master		+= $orderPay_amount;
		        }
		        if($orderPay_meth_key == "5") {		// Cheque;
			        $total_cheque 		+= $orderPay_amount;
		        }
		        
		        $total_amt 				+= $orderPay_amount;
	        }
	        unset($orderRows);
	        unset($orderResult);
	        
	        unset($orderPay_amount);
	        unset($orderPay_meth_key);
	        unset($orderPay_order_id);
        }
        unset($orderPayData);
        unset($orderPayResult);
