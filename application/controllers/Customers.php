...
class Customers extends CI_Controller
{
    ...
    public function exportCustomer()
    {
        ...
        $sheet->setCellValue('A1', "$lang_customer_report");
        apply_excel_headers_style($sheet, 4);

        $sheet->setCellValue('A2', "$lang_customer_fullname");
        $sheet->setCellValue('B2', "$lang_cust_email");
        $sheet->setCellValue('C2', "$lang_cust_mobile");
        $sheet->setCellValue('D2', "$lang_total_orders");
        $sheet->setCellValue('E2', "$lang_total_spent_amt ($site_currency)");
        ...
        for ($t = 0; $t < count($custDtaData); ++$t) {
            ...
            $sheet->setCellValue("A$jj", "$cust_fn");
            $sheet->setCellValue("B$jj", "$cust_em");
            $sheet->setCellValue("C$jj", "$cust_mb");
            $sheet->setCellValue("D$jj", "$total_ordered_qty");
            $sheet->setCellValue("E$jj", "$total_ordered_amt");
    ...
    public function exportCustomerHistory()
    {
        ...
        $sheet->setCellValue('A1', "$lang_sales_history : $cust_fn");

        apply_excel_headers_style($sheet, 8);

        $sheet->setCellValue('A2', "$lang_sale_id");
        $sheet->setCellValue('B2', "$lang_type");
        $sheet->setCellValue('C2', "$lang_date_time");
        $sheet->setCellValue('D2', "$lang_products");
        $sheet->setCellValue('E2', "$lang_qty");
        $sheet->setCellValue('F2', "$lang_total_qty");
        $sheet->setCellValue('G2', "$lang_sub_total ($site_currency)");
        $sheet->setCellValue('H2', "$lang_tax ($site_currency)");
        $sheet->setCellValue('I2', "$lang_grand_total ($site_currency)");
        ...
        for ($d = 0; $d < count($orderData); ++$d) {
            ...
            $sheet->setCellValue("B$jj", "$type_name");
            $sheet->setCellValue("C$jj", "$ordered_dtm");

            if (count($pcodeArray) > 0) {
                $f_pcode = '';
                $f_pcode = $pnameArray[0].' ['.$pcodeArray[0].']';
                $sheet->setCellValue("D$jj", "$f_pcode");
            } else {
                $sheet->setCellValue("D$jj", '');
            }

            if (count($qtyArray) > 0) {
                $f_qty = '';
                $f_qty = $qtyArray[0];
                $sheet->setCellValue("E$jj", "$f_qty");
            } else {
                $sheet->setCellValue("E$jj", '');
            }

            $sheet->setCellValue("F$jj", "$total_item_qty");
            $sheet->setCellValue("G$jj", "$subTotal");
            $sheet->setCellValue("H$jj", "$gstTotal");
            ...
            if (count($pcodeArray) > 1) {
                for ($p = 1; $p < count($pcodeArray); ++$p) {
                    ...
                    $sheet->setCellValue("A$jj", '');
                    $sheet->setCellValue("B$jj", '');
                    $sheet->setCellValue("C$jj", '');
                    $sheet->setCellValue("D$jj", "$s_pcode");
                    $sheet->setCellValue("E$jj", "$s_qty");
                    $sheet->setCellValue("F$jj", '');
                    $sheet->setCellValue("G$jj", '');
                    $sheet->setCellValue("H$jj", '');
                    $sheet->setCellValue("I$jj", '');
        ...
        $sheet->setCellValue("G$jj", "$total_sub_amt ($site_currency)");
        $sheet->setCellValue("H$jj", "$total_tax_amt ($site_currency)");
        $sheet->setCellValue("I$jj", "$total_grand_amt ($site_currency)");
        ...
        $writer = new Xlsx($spreadsheet);
        $filename = 'Sales_History_for_'.$cust_fn;
