...
class Expenses extends CI_Controller
{
    ...
    public function view()
    {
        ...
        if ($page == 0) {
            ...
            $sh_text = "Showing $start_point to ".count($data['results']).' of '.$this->Expenses_model->record_expenses_count().' entries';
        } else {
            $start_sh = $page + 1;
    ...
    public function exportExpenses()
    {
        ...
        $sheet->setCellValue('A1', "$lang_expenses_report");

        apply_excel_headers_style($sheet, 5);

        $sheet->setCellValue('A2', "$lang_expenses_numb");
        $sheet->setCellValue('B2', "$lang_expenses_category");
        $sheet->setCellValue('C2', "$lang_outlet");
        $sheet->setCellValue('D2', "$lang_expenses_date");
        $sheet->setCellValue('E2', "$lang_reason");
        $sheet->setCellValue('F2', "$lang_amount ($site_currency)");
        ...
        for ($e = 0; $e < count($expData); ++$e) {
            ...
            $sheet->setCellValue("A$jj", "$exp_numb");
            $sheet->setCellValue("B$jj", "$exp_cat_name");
            $sheet->setCellValue("C$jj", "$outlet_name");
            $sheet->setCellValue("D$jj", "$exp_date");
            $sheet->setCellValue("E$jj", "$exp_reason");
            $sheet->setCellValue("F$jj", "$exp_amt");
        ...
        $sheet->mergeCells("A$jj:E$jj");
        $sheet->setCellValue("A$jj", "$lang_total");
        $sheet->setCellValue("F$jj", "$total_exp_amt ($site_currency)");
