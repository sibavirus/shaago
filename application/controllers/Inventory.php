...
class Inventory extends CI_Controller
{
    ...
    public function view()
    {
        ...
        if ($page == 0) {
            ...
            $sh_text = "Showing $start_point to ".count($data['results']).' of '.$this->Inventory_model->record_product_count().' entries';
        } else {
            $start_sh = $page + 1;
    ...
    public function exportInventory()
    {
        ...
        $sheet->setCellValue('A1', "$lang_inventory_report");

        $sheet->setCellValue('A2', "$lang_pcode");
        $sheet->setCellValue('B2', "$lang_pname");
        $sheet->setCellValue('C2', "$lang_outlet_name");
        $sheet->setCellValue('D2', "$lang_quantity");

        apply_excel_headers_style($sheet, 3);
        ...
        for ($p = 0; $p < count($prodData); ++$p) {
            ...
            for ($t = 0; $t < count($outletData); ++$t) {
                ...
                if ($t == 0) {
                    $sheet->setCellValue("A$jj", "$pcode");
                    $sheet->setCellValue("B$jj", "$name");
                }
                $sheet->setCellValue("C$jj", "$outlet_name");
                $sheet->setCellValue("D$jj", "$invQty");

                ++$jj;

                apply_excel_content_row_style($sheet, $jj, 3);
        ...
        $sheet->mergeCells("A$jj:C$jj");
        $sheet->setCellValue("A$jj", "$lang_total");
        $sheet->setCellValue("D$jj", "$total_stock_qty");

        apply_excel_headers_style($sheet, 3, $jj);
