...
class Debit extends CI_Controller
{
    ...
    public function view()
    {
        ...
        if ($page == 0) {
            ...
            $sh_text = "Showing $start_point to ".count($data['results']).' of '.$this->Debit_model->record_debit_count().' entries';
        } else {
            $start_sh = $page + 1;
    ...
    public function submitDebitPayment() {
        if(isset($_POST["id"])) {
			...
			$cheque 			= strip_tags($this->input->post('cheque'));
			$addi_card_numb 	= strip_tags($this->input->post('add_card_numb'));
			...
			if($payNameRows == 1) {
				...
				$pay_meth_name	= $payNameData[0]->name;
				unset($payNameData);
			}
			unset($payNameRows);
			unset($payNameResult);
			...
			unset($loadPayAmtData);
			unset($loadPayAmtResult);
			
			$total_paid_amt 	= $curr_total_paid_amt + $payment_amount;
			
			$total_paid_amt 	= number_format($total_paid_amt, 2, '.', '');
			$order_grandTotal 	= number_format($order_grandTotal, 2, '.', '');
			
			if($total_paid_amt < $order_grandTotal) {
				$vt_status 		= "0";
			} else {
				$vt_status 		= "1";
			}
			...
			// Order Payment -- START;
			$order_pay_number 			= "";
            if( ($payment_method == "3") || ($payment_method == "4")) {		// VISA, MASTER;
	            $order_pay_number 		= $addi_card_numb;
            } else if ($payment_method == "5") {							// Cheque;
	            $order_pay_number 		= $cheque;
            } else {
	            $order_pay_number 		= "";
            }
            
			$ins_order_payment_data 	= array(
					"order_id"				=>	$order_id,
					"payment_method_id"		=>	$payment_method,
					"payment_method_name"	=>	$pay_meth_name,
					"payment_amount"		=>	$payment_amount,
					"number"				=>	$order_pay_number,
					"created_user_id"		=>	$us_id,
					"created_datetime"		=>	$tm,
					"status"				=>	"1"
			);
			$this->Constant_model->insertData("order_payments", $ins_order_payment_data);
			// Order Payment -- END;
			
			$upd_order_data 	= array(
					"payment_method"		=>	$payment_method,
					"payment_method_name"	=>	$pay_meth_name,
					"cheque_number"			=>	$cheque,
					"paid_amt"				=>	$total_paid_amt,
					"updated_user_id"		=>	$us_id,
					"updated_datetime"		=>	$tm,
					"vt_status"				=>	$vt_status,
					"card_number"			=>	$addi_card_numb
			);
	        if ($this->Constant_model->updateData("orders", $upd_order_data, $order_id)) {
    ...
    public function exportDebit()
    {
        ...
        $lang_unpaid_amount 		= $this->lang->line("unpaid_amount");
        ...
        $sheet->setCellValue('A1', "$lang_debit_report");
        apply_excel_headers_style($sheet, 5);

        $sheet->setCellValue('A2', "$lang_sale_id");
        $sheet->setCellValue('B2', "$lang_date");
        $sheet->setCellValue('C2', "$lang_outlet_name");
        $sheet->setCellValue('D2', "$lang_customer");
        $sheet->setCellValue('E2', "$lang_grand_total");
        $sheet->setCellValue('F2', "$lang_unpaid_amount");
        ...
        $orderResult = $this->db->query("SELECT * FROM orders WHERE vt_status = '0' AND status = '1' ORDER BY id DESC ");
        ...
        for ($d = 0; $d < count($orderData); ++$d) {
            ...
            $total_unpaid_amt	+= $unpaid_amt;

            $sheet->setCellValue("A$jj", "$order_id");
            $sheet->setCellValue("B$jj", "$order_date");
            $sheet->setCellValue("C$jj", "$outlet_name");
            $sheet->setCellValue("D$jj", "$cust_name");
            $sheet->setCellValue("E$jj", "$grandTotal");
            $sheet->setCellValue("F$jj", "$unpaid_amt");
            ...
            $sheet->getRowDimension("$jj")->setRowHeight(20);
        ...
        $sheet->setCellValue("E$jj", "$total_amt");
        $sheet->setCellValue("F$jj", "$total_unpaid_amt");

        apply_excel_headers_style($sheet, 5, $jj);

        $sheet->getRowDimension("$jj")->setRowHeight(30);

        $writer = new Xlsx($spreadsheet);
        $filename = 'Debit_Report';
